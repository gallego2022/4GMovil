...
- **Problema**: El modelo `EspecificacionCategoria` usa el campo `estado`, pero el controlador y vistas usaban `activo`
- **Correcciones realizadas**:
  - ✅ `getByCategoria()`: Cambiado `activo` por `estado` (línea 237)
  - ✅ `toggleEstado()`: Cambiado `activo` por `estado` (líneas 261, 264, 274)
  - ✅ `store()`: Agregada validación de `estado` (línea 67)
  - ✅ `update()`: Agregada validación de `estado` (línea 162)
  - ✅ `show.blade.php`: Cambiado `$especificacion->activo` por `$especificacion->estado` (líneas 268, 269, 331, 343)
  - ✅ `edit.blade.php`: Cambiado `activo` por `estado` en input y label (línea 197)
  - ✅ `create.blade.php`: Cambiado `activo` por `estado` en input y label (línea 184)
  - ✅ JavaScript en `show.blade.php`: Corregido error de sintaxis y cambiado `activo` por `estado` (línea 343)
