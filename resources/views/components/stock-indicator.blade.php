...
<div class="flex flex-col space-y-2">
    ...
    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
        ...
        <!-- Stock Disponible -->
        <div class="h-2 rounded-full transition-all duration-300 {{ $disponible > 10 ? 'bg-green-500' : ($disponible > 5 ? 'bg-yellow-500' : 'bg-red-500') }}" 
             style="width: {{ $porcentajeDisponible }}%"></div>
        
        <!-- Stock Reservado -->
        @if($reservado > 0)
        <div class="h-2 bg-blue-500 rounded-full transition-all duration-300" 
             style="width: {{ $porcentajeReservado }}%"></div>
        @endif
    </div>
    ...
    <div class="grid grid-cols-3 gap-2 text-xs">
        ...
        <div class="text-center">
            <div class="font-medium text-gray-900 dark:text-gray-100">{{ $totalStock }}</div>
            <div class="text-gray-500 dark:text-gray-400">Total</div>
        </div>
        
        <!-- Disponible -->
        <div class="text-center">
            <div class="font-medium {{ $disponible > 10 ? 'text-green-600 dark:text-green-400' : ($disponible > 5 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400') }}">
                {{ $disponible }}
            </div>
        ...
        <!-- Reservado -->
        <div class="text-center">
            <div class="font-medium {{ $reservado > 0 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500' }}">
                {{ $reservado }}
            </div>
    ...
    @elseif($reservado > $totalStock * 0.5)
