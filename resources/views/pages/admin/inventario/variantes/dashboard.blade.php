...
@section('title', 'Dashboard de Variantes - 4GMovil')
    ...
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.inventario.variantes.movimientos') }}" 
                ...
                <a href="{{ route('admin.inventario.variantes.reporte') }}" 
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $reporteVariantes['total_variantes'] ?? 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $reporteVariantes['variantes_con_stock'] ?? 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $reporteVariantes['variantes_sin_stock'] ?? 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $reporteVariantes['variantes_necesitan_reposicion'] ?? 0 }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                <p class="text-3xl font-bold text-green-600 dark:text-green-400">
                    ${{ number_format($reporteVariantes['valor_total_inventario'] ?? 0, 0, ',', '.') }}
                </p>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Stock total: {{ number_format($reporteVariantes['stock_total'] ?? 0, 0, ',', '.') }} unidades
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2 py-1 text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 rounded-full">
                    {{ $variantesStockBajo->count() }} variantes
                </span>
            </div>
                    ...
                    @foreach($variantesStockBajo->take(5) as $variante)
                        <div class="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                            <div class="flex items-center space-x-3">
                                @if($variante->imagenes->isNotEmpty())
                                    <img src="{{ asset('storage/' . $variante->imagenes[0]->ruta_imagen) }}" 
                                ...
                                <div>
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ $variante->producto->nombre_producto }}
                                    </div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ $variante->nombre }} - ID: {{ $variante->variante_id }}
                                    </div>
                                </div>
                            </div>
                            <div class="text-right">
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 rounded-full">
                    {{ $variantesSinStock->count() }} variantes
                </span>
            </div>
            
            @if($variantesSinStock->count() > 0)
                <div class="space-y-3">
                    @foreach($variantesSinStock->take(5) as $variante)
                    ...
                    @endforeach
                    @if($variantesSinStock->count() > 5)
                        <div class="text-center">
                            <a href="#" class="text-sm text-purple-600 dark:text-purple-400 hover:underline">
                                Ver {{ $variantesSinStock->count() - 5 }} más...
                            </a>
                        </div>
                    @endif
                </div>
