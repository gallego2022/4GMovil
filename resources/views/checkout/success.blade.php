            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        @endphp
                        <span
                            class="inline-flex items-center rounded-md {{ $estadoClasses['bg'] }} px-2 py-1 text-xs font-medium {{ $estadoClasses['text'] }} ring-1 ring-inset {{ $estadoClasses['ring'] }}">
                    ...
                    <div class="space-y-4">
                        @foreach ($detalles as $detalle)
                            @php
                                ...
                                $variante = $detalle->variante_id
                                    ? \App\Models\VarianteProducto::find($detalle->variante_id)
                                    : null;
                            @endphp

                            <div
                                class="flex items-center justify-between py-3 border-b border-gray-200 dark:border-gray-600 last:border-b-0">
                                <div class="flex items-center space-x-4">
                                    <div class="flex-shrink-0">
                                        ...
                                        @else
                                            <div
                                                class="h-16 w-16 rounded-lg bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                                    ...
                                    <div>
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $producto->nombre_producto }}</h4>
                                        @if ($variante)
                                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $variante->nombre }}</p>
                                        @endif
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Cantidad:
                                            {{ $detalle->cantidad }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                                        ${{ number_format($detalle->subtotal, 0, ',', '.') }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        ${{ number_format($detalle->precio_unitario, 0, ',', '.') }} c/u</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            @if ($pedido->direccion)
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                        <p class="font-medium text-gray-900 dark:text-white">{{ $pedido->direccion->nombre_destinatario }}
                        </p>
                        ...
                        @if ($pedido->direccion->referencias)
                            <p class="text-gray-500 dark:text-gray-400 text-sm mt-2">
                                <strong class="text-gray-700 dark:text-gray-300">Referencias:</strong>
