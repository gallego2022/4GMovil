...
class AuthController extends WebController
{
    ...
    public function __construct(AuthService $authService, JwtService $jwtService, CarritoService $carritoService)
    {
    ...
    public function logear(Request $request)
    {
        ...
        try {
            ...
            if ($result['success']) {
                // Sincronizar carrito de sesión con el carrito del usuario
                try {
                    $this->carritoService->syncSessionCartWithUser();
                } catch (\Exception $e) {
                    Log::warning('Error al sincronizar carrito después del login: '.$e->getMessage());
                    // Continuar aunque falle la sincronización del carrito
                }
            ...
            if ($usuario) {
                ...
                if ($usuario->google_id === $googleUser->getId()) {
                    ...
                    try {
                        ...
                        Log::warning('Error al sincronizar carrito después del login con Google: '.$e->getMessage());
                    }
        ...
        if ($token && $token !== 'otp') {
            try {
                ...
                if ($decoded && strpos($decoded, '|') !== false) {
                    [$emailFromToken, $timestamp] = explode('|', $decoded, 2);
