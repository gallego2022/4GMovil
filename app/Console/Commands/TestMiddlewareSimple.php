...
class TestMiddlewareSimple extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'middleware:test-simple {--middleware= : Test specific middleware}';
     ...
     * @var string
     */
    protected $description = 'Simple test for all registered middlewares';
    ...
    {
        $this->info('🧪 Prueba simple de middlewares...');
        $this->newLine();
    ...
    private function testAllMiddlewares()
    {
        $tests = [
            ...
            'VerifyCsrfToken' => [$this, 'testVerifyCsrfToken']
        ];
    ...
    private function testSpecificMiddleware($middlewareName)
    {
        ...
        if (isset($testMap[$middlewareName])) {
            $result = call_user_func($testMap[$middlewareName]);
