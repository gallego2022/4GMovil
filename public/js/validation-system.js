...
class ValidationSystem {
    ...
    setupDefaultValidators() {
        ...
        // Validador específico para precios colombianos
        this.addValidator('priceColombian', (value) => {
            const num = parseFloat(value);
            return num >= 10000 && num <= 20000000;
        });
    ...
    /**
     * Agregar mensajes para un campo
     */
    ...
    setupEventListeners() {
        document.addEventListener('DOMContentLoaded', () => {
            // Validación en tiempo real para inputs que no usan el componente x-validation-field
            document.querySelectorAll('input:not([data-field-name]), textarea:not([data-field-name]), select:not([data-field-name])').forEach(field => {
     ...
     */
    async validateField(field, customRules = null, messages = {}) {
        const fieldName = field.name;
        const value = field.value;
        const rules = customRules || this.rules.get(fieldName) || [];
        ...
        for (const rule of rules) {
            ...
            const result = await this.applyRule(field, rule, value, messages);
            if (!result.isValid) {
                isValid = false;
    ...
    getRequiredMessage(fieldName) {
        const messages = this.messages.get(fieldName) || {};
        return messages.required || `${this.getFieldLabel(document.querySelector(`[name="${fieldName}"]`))} es requerido`;
    }
    ...
    async applyRule(field, rule, value, messages = {}) {
        ...
        // Regla precio colombiano
        if (rule === 'priceColombian') {
